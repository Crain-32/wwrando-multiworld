...
item_type_to_name: Dict[int, AnyStr] = {
...
class GameItem:
    ...
    world_id: int = -1
    item_type: int = ITEM_IS_UNCATEGORIZED
    ...
    def soft_equality(self, game_item: GameItem) -> bool:
        """
        Couldn't remember how to reference __eq__ in a filter, so using this instead.
        """
        return self.game_item_id == game_item.game_item_id and self.world_id == game_item.world_id

    def simple_offset_rep(self) -> AnyStr:
        """
        Since Users don't use a Zero-based index, we use this for In-Game References and the Spoiler Log
        """
        return f"W{self.world_id + 1} {item_id_to_name_dict[self.game_item_id]}"

    def set_item_type(self) -> GameItem:
...
def is_chart(item: GameItem) -> bool:
...
junk_ids: List[int] = [0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x0F, 0x1F, 0x45, 0x46, 0x47, 0x48, 0x82, 0x83, 0xB8]
...
treasure_chart_ids: List[int] = list(range(0xCC, 0xDB))
treasure_chart_ids.extend(list(range(0xDD, 0xF7)))

triforce_chart_ids = list(range(0xF7, 0xFF))

dungeon_item_ids: List[int] = [0x13, 0x14, 0x40, 0x1D, 0x5C, 0x5B, 0x74, 0x73, 0x81, 0x77]
